...
//static ssize_t set_dev_reg_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);

static ssize_t set_dev_reg_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (dev_srv != NULL)
	{
		...
		ret = i2c_smbus_write_byte_data(dev_srv->p_dev_ftm->i2c_client, data[0], data[1]);
		if (!ret)
		{
...
static struct kobj_attribute set_dev_reg = {
	.attr = {"set_reg", 0664},
	//.show = set_dev_reg_show,
	.store = set_dev_reg_store,
};
//static ssize_t get_dev_reg_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
...
int register_single_dev_ftm(struct dev_ftm *client)
{
	...
	if (client->attrs != NULL)
	{
		...
		{
			ret = sysfs_create_file(dev_dir->pkobj, client->attrs[i]);
			if (ret != 0)
			{
